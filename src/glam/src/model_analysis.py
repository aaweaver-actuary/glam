...
class ModelAnalysis:
    def __init__(
        ...
        result: BaseModelResult | None = None,
        ...
        self._result = result if result is not None else StatsmodelsGlmResult(data)

        self._features = features if features is not None else []
        self._interactions = interactions if interactions is not None else []

        self._splitter = splitter if splitter is not None else TimeSeriesDataSplitter()
        self._preprocessor = (
            preprocessor if preprocessor is not None else DefaultPreprocessor(data.df)
        )
    ...
    @property
    def result(self) -> BaseModelResult:
        return self._result
    ...
    @property
    def resid(self) -> dict[str, np.array]:
        """Return the residuals of the model."""
        dat, names = [], []
        for i, model in enumerate(self.models.model_generator):
            dat.append(self.result.residuals(model))
            names.append(i + 1)

        return dict(zip(names, np.array(dat)))
    ...
    def accuracy(self) -> dict[int, float]:
        ...
        for i, model in enumerate(self.models.model_generator):
            ...
            y_pred = self.result.yhat(X, model).round(0)
    ...
    def auc_roc(self) -> dict[int, float]:
        ...
        try:
            ...
            for i, model in enumerate(self.models.model_generator):
                X = self.X.loc[self.cv == i + 1]
                y = self.y[self.cv == i + 1]
                dat.append(self.result.roc_auc(X, y, model))
                names.append(i + 1)
    ...
    def ap(self) -> dict[int, float]:
        ...
        try:
            ...
            for i, model in enumerate(self.models.model_generator):
                X = self.X.loc[self.cv == i + 1]
                y = self.y[self.cv == i + 1]
                dat.append(self.result.ap(X, y, model))
                names.append(i + 1)
    ...
    def log_loss(self) -> dict[int, float]:
        ...
        try:
            ...
            for i, model in enumerate(self.models.model_generator):
                X = self.X.loc[self.cv == i + 1]
                y = self.y[self.cv == i + 1]
                dat.append(self.result.log_loss(X, y, model))
                names.append(i + 1)
    ...
    def breier(self) -> dict[int, float]:
        ...
        try:
            ...
            for i, model in enumerate(self.models.model_generator):
                X = self.X.loc[self.cv == i + 1]
                y = self.y[self.cv == i + 1]
                dat.append(self.result.breier(X, y, model))
                names.append(i + 1)
    ...
    def aic(self) -> dict[int, float]:
        ...
        try:
            ...
            for i, model in enumerate(self.models.model_generator):
                dat.append(self.result.aic(model))
                names.append(i + 1)
    ...
    def bic(self) -> dict[int, float]:
        ...
        try:
            ...
            for i, model in enumerate(self.models.model_generator):
                dat.append(self.result.bic(model))
                names.append(i + 1)
    ...
    def deviance(self) -> dict[int, float]:
        ...
        try:
            ...
            for i, model in enumerate(self.models.model_generator):
                dat.append(self.result.deviance(model))
                names.append(i + 1)
    ...
    def mallow_cp(self) -> dict[int, float]:
        ...
        try:
            ...
            for i, model in enumerate(self.models.model_generator):
                dat.append(self.result.mallow_cp(model))
                names.append(i + 1)
